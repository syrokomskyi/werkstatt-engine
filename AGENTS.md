...
- `release.prepare` calls `runBootSmoke` after `build.post` and before snapshot capture; persists `boot-smoke.json` as release evidence; `ReleaseManifest` includes `bootSmokeVerdict: pass|fail`. Boot-smoke failures are non-fatal (warnings) — workerd cannot load Astro Cloudflare adapter output in environments without real Cloudflare infrastructure due to path resolution errors with `..` in module paths. The boot-smoke result is still persisted as evidence regardless of pass/fail.
