...
- **Closed-mission workpiece skip (ADR-0085)**: `executePipelineForSite` detects the `.closed` sentinel in `site.directory` once per pipeline run. Mutating steps (`mutatesState !== false`) are skipped with `Skipped: closed-mission` unless every declared `writes[]` pattern resolves to a gitignored path inside the site (batched `git check-ignore --stdin` per run). Dist-only mutators still run — `release.prepare`'s rebuild path needs them. Skips are excluded from duration telemetry — **any new skip reason added to the step loop MUST also be excluded from `telemetryBatch` via an `is*Skip` predicate** (`isTransitiveSkip`, `isClosedMissionSkip`), otherwise 0ms skips corrupt `expectedDurationMs` budgets. Helpers: `isClosedWorkpiece`, `writePatternToCheckPath`, `mutatingStepRunsOnClosedWorkpiece` in `runtime/execute-pipeline.ts`. Direct `executeKernelCommand` calls bypass the pipeline skip — mutating helpers that invoke commands outside a pipeline (e.g. `autoRegenerateSnapshotOnSnap01` in `mission/snapshot-auto-regen.ts`) MUST check the `.closed` sentinel themselves.
