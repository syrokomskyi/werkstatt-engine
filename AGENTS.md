...
- `release.prepare` calls `runBootSmoke` after `build.post` and before snapshot capture; persists `boot-smoke.json` as release evidence; `ReleaseManifest` includes `bootSmokeVerdict: pass|fail`. Assets directory from wrangler config is resolved to an absolute path before passing to Miniflare — workerd rejects relative paths containing `..` (e.g. `../client` in Astro-generated `wrangler.json`).
