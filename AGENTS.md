...
- **Every `*.validate` / `*.check` / `*.lint` command MUST declare `contract` and `rules` in its `KernelCommandDefinition` (RFC-0963, DNA-91).** `validator.inventory.generate` fails closed on untagged validators — one missing tag blocks inventory regeneration for the whole workspace. Use `rules: []` for validators that emit no ruleIds. Discovered 2026-09-16: `lagebild.validate` and `forge.public-surface.validate` were untagged and blocked the inventory.
- **Adding a blocking preflight/gate to a command requires seeding its prerequisites in existing test fixtures' `beforeEach`.** Tests that exercise later checks will fail on the new gate unless the fixture satisfies it first. Discovered during RFC-1125: adding an identity+signing-key preflight to `sternsystem.register` broke 7 existing tests that exercised later validations — each needed `werkstatt.identity.json` + a signing key seeded in `beforeEach` before the new gate would let them through.
- **Test ids for `sternsystem.register` must not end in a TLD-like suffix** (`-site`, `-com`, `-de`, …). The RFC-0902 TLD check fires before other validations, so an id like `test-site` trips it and masks the check under test. Use a plain word like `acme`. Discovered during RFC-1125.
...
- **Closed-mission workpiece skip (ADR-0085, ADR-0087)**: enforcement lives in `executeRegisteredCommand` (`runtime/execute-command.ts`) — the single choke point for pipeline steps AND direct `executeKernelCommand` calls. A mutating command (`mutatesState !== false`) whose `context.site.directory` carries the `.closed` sentinel returns `Skipped: closed-mission` unless every declared `writes[]` pattern resolves to a gitignored path inside the site (`git check-ignore --stdin`). Dist-only mutators still run — `release.prepare`'s rebuild path needs them. `executePipelineForSite` keeps an early skip via the same predicate for clean step reporting; skips are excluded from duration telemetry — **any new skip reason added to the step loop MUST also be excluded from `telemetryBatch` via an `is*Skip` predicate** (`isTransitiveSkip`, `isClosedMissionSkip`), otherwise 0ms skips corrupt `expectedDurationMs` budgets. Helpers: `isClosedWorkpiece`, `writePatternToCheckPath`, `collectGitignoredPaths`, `mutatingStepRunsOnClosedWorkpiece`, `mutatingCommandRunsOnClosedWorkpiece` in `runtime/closed-workpiece.ts`; `skippedExecutionReport` in `runtime/shared.ts`. Direct fs writes outside `executeKernelCommand` (e.g. `writePreliminaryBuildIdentity`) are covered by gitignoring their targets in the workpiece `.gitignore` template.
...
- **`context.actualState` is a single-module registry under the manifest fast-path.** `execute-command.ts` builds `actualState` via `buildRegistryForModule()` when the command manifest knows the owning module — the command sees only its own module's commands. Registry-introspection commands (`validator.inventory.generate`, manifest generators, anything iterating "all commands") MUST build the full workspace registry via `getOrBuildWorkspaceRegistry(context.workspaceRoot)` (cached in `registry-cache.ts`) instead of reading `context.actualState`. Also note: `KernelRegisteredCommandInfo` (from `listRegisteredKernelCommands`) does NOT propagate `contract`/`rules` — use `ActualState.commands` (`KernelCommandDefinition`) when those fields are needed. Discovered 2026-09-16: `validator.inventory.generate` silently produced an empty inventory because it filtered a single-module `actualState`.
- **Mock `KernelRuntimeContext` for `executeRegisteredCommand` tests MUST carry a real IO adapter.** Provide `io: createDefaultIO().io` and `fileIntents: intents` (from `@warpgogol/werkstatt-shared/kernel`) — the executor binds `io` methods when building the effective adapter (`createReadOnlyIO` for `mutatesState: false`, `createRecordingIO` for dry-run), so a missing `io` crashes with `Cannot read properties of undefined` before `execute()` runs. See `runtime/tests/execute-pipeline-closed-mission.test.ts` `makeContext` for the pattern.
- **Universal `--site`/`--system` are consumed by the CLI and re-injected into argv for workspace commands.** `cli/index.ts` parses `--site` into `options.siteName` (and `--system` similarly); `execute-command.ts` then unconditionally re-injects them into `wsArgv` before flag resolution, so `resolveSiteFlagAlias` and command flag parsing see them in `input.argv`. Agents adding flag features MUST NOT re-implement this plumbing or patch the CLI — the alias/flag layer already receives the re-injected tokens. `input.argv` carries the post-alias argv (canonical flags), not the raw CLI args. Discovered during RFC-1126 fo-review.
...
- **Validators with hardcoded package paths rot on package renames.** `join(workspaceRoot, "packages", "<name>", ...)` literals inside validators, their tests, and `reads[]` manifest lists silently break when a package directory is renamed (e.g. `packages/werkstatt` → `packages/werkstatt-engine` in RFC-0769 left SHARED-01/02 failing for months). After any package rename, grep the old directory name across `src/plugin/`, `src/checks/`, `os/*.module.ts`, and `src/tests/` and update every path literal. Discovered during RFC-1104.
...
- ADR-0084: `sweep.ts` (`findIncompleteOperationsForMission`, `abandonIncompleteOperationsForMission`) scans `systems-cache/<id>/operations/*.jsonl` for incomplete ops referencing a mission (op-started `missionId`, opId, or filename inference). `mission.close` runs an `abandon-incomplete-operations` step before `transition-state` that appends `op-abandoned` (reason `mission closed: <id>`) and commits the journal updates to the cache clone. Unreadable journal files that reference the closing mission fail the step closed; unrelated corrupt files warn only.
- ADR-0088: journal contract — every `op-started` in `systems-cache/<id>/operations/*.jsonl` MUST carry a non-empty `missionId` (enforced by blocking `JRN-01` rule in `sternsystem.validate`; unreadable journals warn). Terminal records are idempotent: any `op-done`/`op-abandoned` completes the op, a terminal after a terminal is legal (in-flight `leitstand.ship` gets `op-abandoned` during its own close step, then `op-done` on success). `fleet/operations/` is out of scope — fleet ops are not mission-scoped.
- **Never match IDs inside larger strings with raw `includes()`** — `m-006` is a substring of `m-0060`, so prefix-extension false-positives abandon/match the wrong entity. Use a boundary-aware check: the ID must not be followed by `[0-9A-Za-z]` (see `referencesMissionId` in `src/journal/sweep.ts`). Applies to missionIds, opIds, systemIds in filenames, opIds, and journal records.
- **When committing swept/generated files to the cache clone, `git add` explicit file paths — never `git add <dir>/`.** A directory add stages unrelated dirty files (e.g. another mission's in-flight journal) into the commit. Collect the exact paths the step modified and add each one (see `abandon-incomplete-operations` in `mission-close.ts`).
...
- RFC-1028: incremental mission validation with persistent cache. `moduleBasePath` on `KernelRegisteredCommandInfo` is derived from `command.modulePath` (RFC-0960) in `commandInfo()` for dynamic `moduleSrcDir` resolution in the pipeline executor. `buildCommandResultCacheKey` uses a composite string (`schemaVersion:commandName:siteName:inputsHash:moduleHash:flagsHash` since RFC-1133 schema v2) instead of `stableJsonHash` — enables SQL LIKE filtering by `commandName` in `CacheLayer.list()`. `validation.state.inspect` command (read-only) reads `.validation-state.json` and the SQLite cache. `.validation-state.json` is gitignored at `missions/<missionId>/.validation-state.json`. `mission.validate --force` bypasses the cache; agents MUST NOT delete `.validation-state.json`.
- RFC-1130 + RFC-1133: `cacheBypassFlags?: string[]` on `KernelCommandDefinition` — flag names that bypass the result cache when present in argv (`--name` or `--name=value`). Flags that reduce coverage (e.g. `--rfc` on `qa.independent.run`) MUST be listed here so a filtered run can neither serve a stale full-run report nor poison the full-run entry. Enforcement lives in `runtime/result-cache.ts`: `hasCacheBypassFlag(command, argv)` is evaluated inside `resolveResultCache` (no layer opened) and again inside `readCommandResult`/`writeCommandResult` — uniform on the direct path and both pipeline executors. Unlike `--force`, bypass never refreshes the entry.
- RFC-1133: the `command_results` read/write lives inside `executeRegisteredCommand` (`runtime/execute-command.ts`) — the single choke point for direct `executeKernelCommand` calls AND both pipeline executors. Pipeline executors no longer call cache helpers; they pass per-run structures via `context.resultCache` (`{ layer, treeIndex, moduleHashCache }`, type `KernelResultCacheContext` in `@warpgogol/werkstatt-shared/kernel`). `executeKernelCommand` shares one lazily-created layer across `--all` sites; a bare `executeRegisteredCommand` call without the carrier opens and closes its own layer. The cache key carries `flagsHash` — `stableJsonHash` over resolved `input.flags` minus `CACHE_INVISIBLE_FLAGS` (`json`, `quiet`, `verbose`, `help`, `dry-run`, `force`, `root`) — computed post flag-alias resolution, so `--site X` and its canonical flag share one entry while coverage flags (`--id`, `--rfc` values, …) split it. `--force` skips the read but still writes (refresh, DNA-93); `--dry-run` skips both. Cache read/write failures degrade to fresh execution, never fail the command.
- **Semantic fingerprinting strips comments — comment-only changes never invalidate `inputsHash`/`moduleHash`.** `.ts`/`.tsx`/`.astro`/`.css`/`.js`/`.mjs` inputs are fingerprinted in `semantic` mode (`selectFingerprintMode` in `command-result-cache.ts`); pragmas carried in comments (e.g. `// boot-test: exempt-binding <name>` consumed by `binding-validator.ts`) are invisible to the cache key. Two consequences: (a) cache-contract test fixtures MUST invalidate via structural changes (added statements/exports), not comment edits — comment-only fixture edits produce identical hashes and false test failures; (b) never design a comment-carried pragma as the sole trigger for a cacheable command's behavior — put it in a byte-hashed surface (config field, frontmatter) instead. Discovered during RFC-1132 (fo-review REVIEW-CODE-2026-09-22-01).
- **`command_results` composite keys embed `algo:hex`-prefixed hashes — never parse them with naive `split(":")`.** `computeModuleHash`/`computeInputsHash` return `sha256:<hex>` digests, so a real key is `2:cmd:site:sha256:<IH>:sha256:<MH>:<FH>` (8 segments; `<FH>` is the flagsHash, empty when no cache-visible flags). Always use `parseCommandResultCacheKey` — it parses the three hashes from the right with algo-prefix detection. Discovered during RFC-1127: the pre-fix parser reported `inputsHash: "sha256"` and `moduleHash: <inputs hex>` for every real entry.
- **Incremental commands without state files locate their predecessor report via `CacheLayer.list()` filtered by `moduleHash`** (pattern established by RFC-1127, `mobile.layout.check`): `mutatesState: false` commands cannot persist a predecessor pointer, so they list `command_results` entries for their command name + site, filter to entries whose `moduleHash` matches the current module fingerprint, and take the most recent. Cache helpers are exported via the `@warpgogol/werkstatt-engine/kernel/cache` subpath (`createCacheLayer`, `computeModuleHash`, `buildCommandResultCacheKey`, `COMMAND_RESULT_CACHE_*`).

### Notausgang export (RFC-0359, RFC-0380, RFC-1120)

- `src/notausgang/` owns the exit-package commands: `notausgang.export` (staging → atomic publish) and `notausgang.validate` (deep integrity verification).
- RFC-1120 package additions: `evidence/<source-id>/` (R2 artifacts + `integrity.txt`), `feature-notes.md`, `verify.py` + `verification.md` (copied verbatim from `src/notausgang/templates/`), `SHA256SUMS.txt` (sha256sum-format, all files except itself), and `notausgang-manifest.json` (JSON twin of the YAML manifest — the old `legacy-json-artifact` rule no longer applies to it).
- Evidence bundling (`evidence-bundle.ts`) is fail-closed: eligible evidence (published claim ref + `publication.visibility: public` + consent scopes via `evaluateGateV2`) whose R2 artifact is unreachable or hash-mismatched aborts the export. Explicit item `r2Path` wins over derived nachweis paths.
- **`evaluateGateV2` throws `UnsupportedNachweisKindError` for kinds outside `NACHWEIS_KIND_TO_POLICY`.** Callers evaluating eligibility for non-Nachweis evidence-source kinds must catch the error (or check kind membership first) — such kinds have no publication policy and no consent aspects, so consent is trivially granted. See `consentScopesGranted` in `evidence-bundle.ts`. Discovered during RFC-1120.
- Templates under `src/notausgang/templates/` (`verify.py`, `verification.md`) carry no Compass headers — Compass `fileExtensions` covers only `.ts/.tsx/.astro/.js/.mjs/.css`, so `.py`/`.md` template files are outside its scan surface.
- Feature inventory (`feature-notes.ts`) unions `system.md` frontmatter, `system-config.yaml` deployment, content-tree markers, `dist/_worker.js`, and opportunistic `entitlements.generated.yaml`; unknown features default to `needs-backend`.
- `notausgang.validate` declares `NA-EVIDENCE-01/02`, `NA-FEATURES-01`, `NA-VERIFIER-01`, `NA-MANIFEST-02` in its `rules` array (DNA-91).
...
- **New agent-surface protocol headers must be added to `CORS_HEADERS` in `src/agent-gate/astro.ts`.** Request headers (e.g. `Idempotency-Key`, `MCP-Protocol-Version`, `A2A-Version`) go into `Access-Control-Allow-Headers`; response headers agents must read (e.g. `X-Agent-Idempotency`) go into `Access-Control-Expose-Headers`. A header accepted by the handler but absent from CORS is silently rejected at preflight for browser-based agents — unit tests do not exercise preflight. Discovered during RFC-1112/1113/1114 gap fix.
...
- `readCommandResult` in `runtime/result-cache.ts` (relocated from `tryCacheRead` by RFC-1133) verifies that all files in `command.writes` exist on disk before returning a cached result. If any output file is missing, the cache is treated as a miss and the command re-executes.
...
- `readCommandResult` is exported for testing. Tests: `src/kernel/tests/cache-output-existence.test.ts`.
...
- `mission.validate --fast` (RFC-1086): incremental validation flag that reads the previous `validation-report.json` and skips phases that passed. Mutually exclusive with `--force`. Phase-level skip granularity with cascading re-run: once a phase re-runs, all downstream phases also re-run. Falls back to full run with warning if previous report is missing, corrupt, or has a mismatched missionId. Short-circuits with exit 0 if all phases passed (does NOT overwrite the existing report). `mission.close` does NOT use `--fast` — it always runs full validation.
...
- **`system.md` test fixtures must satisfy `systemManifestSchema` at load time (RFC-1106).** `loadSystemManifest`/`loadSystemManifestSync` parse via Zod — a fixture missing `app`, `version`, or `identity.{systemStar,biome}` fails with ZodError before the code under test runs. When `i18n` is present, `supported` (record) is required alongside `default`. Minimal valid fixture: `app: test-app`, `version: 1.0.0`, `identity: { systemStar: test, biome: default }`, `i18n: { default: de, supported: { de: true } }`. Optional blocks (`growth`, `release`) must be schema-valid when present — `growth.vendor.adapter: null` and `release.passport.heartbeatUrl: ""` both fail parse; omit the blocks instead.
- **Compass tests on temp workspaces must write a fixture `forge.yaml` declaring `bindings.compass.excludedPaths`.** Since RFC-1096, template exclusions (`src/templates/**` → `template-source`) are consumer policy, not generic defaults — `resolveCompassPolicy` on a root without `forge.yaml` yields no exclusions, so `createCompassInventoryEntries` reports template files as `authored` instead of `excluded`.
...
## Flaky test quarantine and retry (RFC-1043)
...
## Test adjacency coverage (RFC-1050, RFC-1051)
...
- **RFC-1051** ratcheted test adjacency for all remaining 291 `werkstatt-engine` source files across 8 groups (A–H): `migrators/`, `fingerprint/`, `integrity/`, `handoff/`, `nachweis/`, `sternsystem/`, `changelog/`, `observability/`, `schemas/`, `agent-gate/`, `bordbuch/`, `deploy/`, `dns/`, `evidence/`, `evolution/`, `identity/`, `isolation/`, `leitstand/`, `plugin/`, `release/`, `runtime/`, `scope/`, `signing/`, `subdomain/`, `werkstatt/`, `workpiece/`, and more. The `.test-adjacency-baseline.json` has 0 untested `werkstatt-engine` files remaining. `agent-gate/astro.ts` is inline-exempted (imports `astro:env/server`, not loadable in vitest).

## Fleet claims registry (RFC-1124)

- **Use `loadSigningKeyFromEnv` from `src/fleet/claims-repo.ts` for env signing-key loading** — the `SIGNING_PRIVATE_KEY`/`SIGNING_PRIVATE_KEY_PATH` → hex-pair pattern was duplicated across 5 call sites (claims-commands, sternsystem-sync, sternsystem-register, mission-close, handover-complete) before extraction. Returns `null` when no key is configured — callers skip silently.
- **Tests creating tmpdir fixtures for sternsystem/fleet code MUST nest the werkstatt root inside the tmpdir** (`join(mkdtempSync(...), "werkstatt")`). `resolveCacheClonePath` resolves `systems-cache` as a _sibling_ of the werkstatt root — a bare tmpdir leaks cache clones into shared `/tmp` and parallel tests interfere. Discovered during RFC-1124: `claims-repo.test.ts` cross-test pollution.
- **When testing mutations of signed artifacts (claims, passports), re-sign after mutating fields.** Mutating a signed claim's `supersededBy` invalidates the signature — the test then verifies `invalid-signature` instead of the intended verdict (`stale`). Strip `signature`, mutate, re-sign via `signClaim`. Discovered during RFC-1124 tombstone test.
