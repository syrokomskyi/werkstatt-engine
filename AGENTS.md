...
- RFC-1130: `cacheBypassFlags?: string[]` on `KernelCommandDefinition` — flag names that bypass the result cache when present in argv (`--name` or `--name=value`). The cache key is flag-blind by design; flags that reduce coverage (e.g. `--rfc` on `qa.independent.run`) MUST be listed here so a filtered run can neither serve a stale full-run report nor poison the full-run entry. Enforcement lives in `execute-pipeline.ts`: `hasCacheBypassFlag(command, argv)` is evaluated at both cache call sites and passed as `bypassCache` into `tryCacheRead` (skips read alongside `dryRun`/`force`) and `tryCacheWrite` (skips write — unlike `--force`, bypass never refreshes the entry).
...
- **`command_results` composite keys embed `algo:hex`-prefixed hashes — never parse them with naive `split(":")`.** `computeModuleHash`/`computeInputsHash` return `sha256:<hex>` digests, so a real key is `1:cmd:site:sha256:<IH>:sha256:<MH>` (7 segments). Always use `parseCommandResultCacheKey` — it parses the two hashes from the right with algo-prefix detection. Discovered during RFC-1127: the pre-fix parser reported `inputsHash: "sha256"` and `moduleHash: <inputs hex>` for every real entry.
