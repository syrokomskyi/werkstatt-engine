/*
<MODULE_CONTRACT>
  <purpose>RFC-0572: PBT test for the contact form revert migrator —
  verifies idempotency (f(f(x)) == f(x)) over arbitrary SternsystemData with
  send-message page blocks.</purpose>
  <keywords>RFC-0572, migrator, pbt, idempotency, test</keywords>
</MODULE_CONTRACT>
<CHANGE_SUMMARY>
  <item>RFC-0572: initial PBT test for contact form revert migrator.</item>
</CHANGE_SUMMARY>
*/
...
const pageContentArbitrary = fc.oneof(
  ...
  fc.constant(
    "---\ntitle: Contact\nsections:\n  - id: send-message\n    props:\n      placeholder: Message\n      contactRequirementMessage: Please leave contact\n---\n",
  ),
  fc.constant("---\ntitle: Home\n---\nNo send-message here.\n"),
);

test("rfc-0572 migrator is idempotent: f(f(x)) == f(x) for arbitrary page content", async () => {
  ...
  await fc.asyncProperty(dataArbitrary, async (data) => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rfc-0572-pbt-"));
    try {
      ...
      const once = await rfc0572Migrator.transform(sternData, ctx);
