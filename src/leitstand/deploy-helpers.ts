...
export async function loadMainVerificationDecision(
  ...
  if (!result.success) {
    ...
    const releasePrefix = path.basename(filePath).replace(/-main-verification\.json$/, "");
    let available: string[] = [];
    try {
      available = (await fs.readdir(dir)).filter((f) => f.startsWith(releasePrefix)).sort();
    } catch {
      /* ignore */
    }
    throw new Error(
      `[deploy] invalid main verification decision: ${result.error.issues.map((i) => i.message).join("; ")}. ` +
        ...
        `Use the file from gate-decisions/${releasePrefix}-main-verification.json (generated by leitstand.certify --gate main), ` +
        `not the gate-decision file.${available.length > 0 ? ` Available files: ${available.join(", ")}` : ""}`,
    );
  }
  return result.data;
}
