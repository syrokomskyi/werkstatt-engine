...
function recordBordbuchCandidateEvent(
  ...
  // Bordbuch integration is handled by the bordbuch module at the kernel level.
  // This function is a placeholder for future bordbuch event recording.
  // The actual bordbuch.append call will be wired in Step 8.
}

export function runCandidateDefine(
  input: KernelCommandInput,
  _context: KernelRuntimeContext,
): KernelCommandResult<{ candidateId: string; stage: string }> {
...
export function runCandidateShadow(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ evidence: CandidateEvidenceV1 | null }> {
  const ctl = getController();
  ...
  ctl
    .shadow(candidateId, metric)
    .then((evidence) => {
      recordBordbuchCandidateEvent(context, candidateId, "shadowing", "");
    })
    .catch(() => {});
  ...
  return {
    data: { evidence: null },
    exitCode: 0,
    summary: `Shadow execution started for candidate ${candidateId}`,
  };
}

export function runCandidateTest(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ evidenceCount: number }> {
  const ctl = getController();
  const candidateId = flagString(input, "candidate-id");
  ...
  ctl
    .test(candidateId, scenarios)
    .then((evidence) => {
      recordBordbuchCandidateEvent(context, candidateId, "testing", "");
    })
    .catch(() => {});
  ...
  return {
    data: { evidenceCount: 0 },
    exitCode: 0,
    summary: `Test execution started for candidate ${candidateId}`,
  };
}

export function runCandidateCanary(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ trafficPercent: number }> {
  const ctl = getController();
  const candidateId = flagString(input, "candidate-id");
  ...
  ctl
    .canary(candidateId, trafficPercent)
    .then(() => {
      recordBordbuchCandidateEvent(context, candidateId, "canary", "");
    })
    .catch(() => {});
...
export function runCandidateActivate(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ candidateId: string; stage: string }> {
  const ctl = getController();
  ...
  ctl
    ...
    .then(() => {
      recordBordbuchCandidateEvent(context, candidateId, "active", "");
    })
    .catch(() => {});
...
export function runCandidatePromote(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ missionId: string; stage: string }> {
  const ctl = getController();
  const candidateId = flagString(input, "candidate-id");
  ...
  ctl
    .promote(candidateId, missionId)
    .then(() => {
      recordBordbuchCandidateEvent(context, candidateId, "promoted", "");
    })
    .catch(() => {});
...
export function runCandidateRollback(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ componentId: string; stage: string }> {
  const ctl = getController();
  const componentId = flagString(input, "component-id");
  ...
  ctl
    ...
    .then(() => {
      recordBordbuchCandidateEvent(context, componentId, "rolled-back", "");
    })
    .catch(() => {});
...
export function runCandidateQuarantine(
  input: KernelCommandInput,
  context: KernelRuntimeContext,
): KernelCommandResult<{ candidateId: string; stage: string; reason: string }> {
  const ctl = getController();
  const candidateId = flagString(input, "candidate-id");
  ...
  ctl
    .quarantine(candidateId, reason)
    .then(() => {
      recordBordbuchCandidateEvent(context, candidateId, "quarantined", "");
    })
    .catch(() => {});
...
export function runCandidateInspect(
  _input: KernelCommandInput,
  _context: KernelRuntimeContext,
): KernelCommandResult<{ candidates: readonly CapabilityCandidateV1[] }> {
  const ctl = getController();
